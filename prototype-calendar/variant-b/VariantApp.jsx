  ...
  const handleDeleteSchedule = (scheduleId) => {
    if (window.confirm('일정을 삭제하시겠습니까?')) {
      setAllSchedules(prev => {
        const updated = (prev[selectedDateKey] || []).filter(schedule => schedule.id !== scheduleId);
        return {
          ...prev,
          [selectedDateKey]: updated,
        };
      });
      setPreparingMsg('일정이 삭제되었습니다.');
      setTimeout(() => setPreparingMsg(null), 2000);
    }
  };
                        ...
                        <div key={s.id} className="bg-white p-5 rounded-[2.5rem] border border-gray-100 flex flex-col gap-4 shadow-sm hover:border-blue-100 transition-colors">
                            <div className="flex justify-between items-center">
                                ...
                                <div className="shrink-0 flex items-center gap-2">
                                  <button onClick={() => handleDeleteSchedule(s.id)} className="w-9 h-9 rounded-full bg-gray-50 border border-gray-100 flex items-center justify-center text-gray-400 active:scale-95 transition-all">
